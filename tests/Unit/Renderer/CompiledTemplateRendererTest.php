...
it('renders from cache', function () {
    ...
    $renderer = new \Phpx\Templates\Renderer\CompiledTemplateRenderer($cache);
...
it('throws if template isn\'t cached', function () {
    ...
    expect(fn() => $renderer->render('foo', new TestTemplateOptions('')))
        ->toThrow(\Phpx\Templates\Exception\TemplateMissingException::class);
});
...
it('throws if template throws', function () {
    ...
    expect(fn() => $renderer->render('foo', new TestTemplateOptions('')))
        ->toThrow(\Phpx\Templates\Exception\TemplateRenderException::class);
});
